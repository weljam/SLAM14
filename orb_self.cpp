...
// 32 bit unsigned int, 将会存储8x32=256
typedef vector<uint32_t> DescType; //描述子
...
//计算描述子
void ComputeORB(const cv::Mat &img, vector<cv::KeyPoint> &keypoints, vector<DescType> &descriptors) {
  const int half_patch_size = 8;//计算特征点方向，选取图像为16*16
  const int half_boundary = 16;//计算描述子时在32*32的图像块中选点
  int bad_points = 0;//计算描述子时，关键点处于边界，周围没有32*32的图像块，舍弃
  for (auto &kp: keypoints) {
    //pt为keypoint成员变量，指向关键点的位置，if中进行关键点是否舍弃的判断
    ...
    //计算16*16图像块的灰度质心
    //计算灰度矩，m01=xI(x,y)  m10 = yI(x,y),m00 = I(x,y)
    float m01 = 0, m10 = 0 ,m00 = 0;
    for (int dx = -half_patch_size; dx < half_patch_size; ++dx) {
      for (int dy = -half_patch_size; dy < half_patch_size; ++dy) {
        //获取该点的灰度大小，y对应的是行数(高)，x对应的是列数（行）
    ...
    //关键点角度计算
    //得到的实际点为C(m10/m00,m01/m00),由于后续要用到sin和cos
    //所以计算斜边长为m_sqrt = sqrt((m10/m00)^2+ (m01/m00)^2)
    //sin = (m01/m00)/m_sqrt可以将m00同时约掉，所以在计算的时候可以将m00省略
    float m_sqrt = sqrt(m01 * m01 + m10 * m10) + 1e-18; //避免零点的出现
    float sin_theta = m01 / m_sqrt; 
    float cos_theta = m10 / m_sqrt;
    // compute the angle of this point
    DescType desc(8, 0);//8个uint32_t存储描述子，全部初始化为0
    for (int i = 0; i < 8; i++) {
      //存储描述子Brief
      uint32_t d = 0;
      for (int k = 0; k < 32; k++) {
        ...
        //旋转，将在从形心坐标系下的坐标点转到OC方向向量坐标系下，所以R=【cos(a) -sin(a);sin(a),cos(a)】
        //注意这里的原点是形心，所以需要加上形心和图像原点的偏移量才是实际的坐标点
...
//特征点匹配，输入图像1描述子和图像二的描述子矩阵（里面存储每个特征点的描述子），这里使用暴力匹配，也可以存成二叉树的形式进行匹配
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
  //设置描述子的最小距离
  const int d_max = 40;
  //遍历图像1和图像2中描述子矩阵
  for (int i1 = 0; i1 < desc1.size(); ++i1) {
    ...
    //定义匹配点，分别存储两幅图像的描述子Index("而描述子的Index对应着关键点的Index")以及汉明距离
    cv::DMatch m{i1, 0, 256}; 
    for (int i2 = 0; i2 < desc2.size(); ++i2) {
      if (desc2[i2].empty()) continue;
      int distance = 0;
      for (int k = 0; k < 8; k++) {
        //_mm_popcnt_u32获取的desc1和desc2异或后的1个数
